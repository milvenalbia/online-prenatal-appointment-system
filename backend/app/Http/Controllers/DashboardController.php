...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        $query = PregnancyTracking::select(
            DB::raw('COUNT(patients.id) AS total_patients'),
            DB::raw('COUNT(pregnancy_trackings.id) AS total_pregnancy_tracking'),
            ...
            DB::raw('COUNT(immuzition_records.id) AS total_immunization'),
            DB::raw('COUNT(prenatal_visits.id) AS total_prenatal_visit'),
            DB::raw('COUNT(out_patients.id) AS total_out_patients'),
        )
